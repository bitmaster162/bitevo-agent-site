...
for (const pattern of [
  ...
  check('routing', runFirst.includes(pattern), `wrangler: missing bounded guide routing pattern ${pattern}`);
}
...
for (const slug of [
  ...
  check('routing', worker.includes(`/guides/${slug}`), `worker: missing canonical guide allowlist ${slug}`);
}
...
for (const required of [
  ...
  "Content-Security-Policy: default-src 'self'; base-uri 'self'; object-src 'none'; frame-ancestors 'none'; form-action 'self'; script-src 'self' 'unsafe-inline'; style-src 'self' 'unsafe-inline'; font-src 'self'; img-src 'self' data:; connect-src 'self'; upgrade-insecure-requests",
...
console.log(`CLOUDFLARE_POLICY_GATE=PASS config=${configChecks} routing=${routingChecks} headers=${headerChecks} failures=0`);
