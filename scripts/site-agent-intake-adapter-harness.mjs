...
const cases = [
  ...
  ['unknown effect denied', { ...base('yakov','trip',{dates:'10 Sep',party:'2',interest:'islands'}), requested_effects:['unknown_future_effect'] }, 'BLOCKED_POLICY'],
  ...
  ['dar L0 handoff denied', { ...base('dar','business',{company:'Cafe'}), requested_effects:['human_handoff'] }, 'BLOCKED_POLICY'],
  ...
  ['wrong schema version denied', { ...base('yakov','trip',{dates:'10 Sep',party:'2',interest:'islands'}), schema_version:'SITE_AGENT_ENVELOPE_V2' }, 'BLOCKED_POLICY'],
  ['extra top-level property denied', { ...base('yakov','trip',{dates:'10 Sep',party:'2',interest:'islands'}), unexpected:true }, 'BLOCKED_POLICY'],
  ['converted browser state denied', { ...base('yakov','trip',{dates:'10 Sep',party:'2',interest:'islands'}), state:'CONVERTED', evidence_refs:['receipt:123'] }, 'BLOCKED_POLICY'],
  ['malformed effects shape denied', { ...base('yakov','trip',{dates:'10 Sep',party:'2',interest:'islands'}), requested_effects:'auto_send' }, 'BLOCKED_POLICY'],
  ['nested field object denied', base('dar','business',{company:{name:'Cafe'}}), 'BLOCKED_POLICY'],
  ['invalid created_at denied', { ...base('yakov','trip',{dates:'10 Sep',party:'2',interest:'islands'}), created_at:'tomorrow' }, 'BLOCKED_POLICY']
];
...
for (const [name, envelope, expected] of cases) {
  ...
  if (ok) passed += 1;
  console.log(`${ok ? 'PASS' : 'FAIL'} ${name} :: ${receipt.decision}${receipt.schema_errors?.length ? ` / schema=${receipt.schema_errors.join(',')}` : ''}`);
}
