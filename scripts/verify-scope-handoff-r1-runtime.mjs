...
{
  const store=new FakeStore(); const p=entry(); const first=await json(await handleScopeHandoffRequest(req(p),{enabled:true,store,idFactory:()=> 'sh_r1_fixed',now:()=> '2026-09-03T00:00:00.000Z'}));
  ...
  const replay=await json(await handleScopeHandoffRequest(req(p),{enabled:true,store,idFactory:()=> 'SHOULD_NOT_MINT'}));
  check(replay.status===200 && replay.body.replayed===true && replay.body.submission_id==='sh_r1_fixed','exact replay same id');
  const conflict=await json(await handleScopeHandoffRequest(req({...p,company:'Changed Co'}),{enabled:true,store})); check(conflict.status===409 && conflict.body.error==='IDEMPOTENCY_CONFLICT','changed payload conflict');
}
{
  const store=new FakeStore(); const out=await json(await handleScopeHandoffRequest(req(primary()),{enabled:true,store})); check(out.status===201,'primary accepted');
}
{
...
}
{
  ...
  const out2=await json(await handleScopeHandoffRequest(req({...entry(),testing_authorization:true}),{enabled:true,store})); check(out2.status===422,'testing authorization cannot be granted');
}
{
  const store=new FakeStore(); const malformed=await json(await handleScopeHandoffRequest(req('{bad'),{enabled:true,store})); check(malformed.status===400,'malformed JSON');
  const type=await json(await handleScopeHandoffRequest(req(JSON.stringify(entry()),{contentType:'text/plain'}),{enabled:true,store})); check(type.status===415,'content type');
  const huge='x'.repeat(MAX_BODY_BYTES+1); const large=await json(await handleScopeHandoffRequest(req(huge),{enabled:true,store})); check(large.status===413,'body ceiling');
}
{
  const store=new FakeStore(); const out=await json(await handleScopeHandoffRequest(req({...entry(),workflow:'Use key sk-proj-ABCDEFGHIJKLMNOPQRSTUVWXYZ1234567890'}),{enabled:true,store})); check(out.status===422 && out.body.error==='SECRET_BOUNDARY_REJECTED','secret pattern');
}
{
  const store=new FakeStore(); const method=await json(await handleScopeHandoffRequest(req(null,{method:'GET'}),{enabled:true,store})); check(method.status===405 && method.headers.get('allow')==='POST','method fail closed');
  const origin=await json(await handleScopeHandoffRequest(req(entry(),{origin:'https://evil.example'}),{enabled:true,store})); check(origin.status===403,'cross origin rejected');
}
{
  const store=new FakeStore(); const p=entry('client_submit_concurrent_01'); const [a,b]=await Promise.all([
    handleScopeHandoffRequest(req(p),{enabled:true,store,idFactory:()=> 'sh_r1_a'}),
    handleScopeHandoffRequest(req(p),{enabled:true,store,idFactory:()=> 'sh_r1_b'})
