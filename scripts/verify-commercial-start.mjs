...
const buildMeasuredValueText = stripTags(buildMeasuredValueGate);
const ruBuildMeasuredValueText = stripTags(ruBuildMeasuredValueGate);
for (const phrase of ['Delivery is not measured value.', 'No delivery or measurement state is represented.', 'Synthetic arithmetic cannot substitute for measured value.']) if (!buildMeasuredValueText.includes(phrase)) failures.push(`/build/measured-value-gate: missing boundary phrase "${phrase}"`);
for (const phrase of ['Выполнение ещё не означает измеренную ценность.', 'Никакой delivery или measurement state здесь не представлен.', 'Synthetic arithmetic не заменяет measured value.']) if (!ruBuildMeasuredValueText.includes(phrase)) failures.push(`/ru/build/measured-value-gate: missing boundary phrase "${phrase}"`);
if (buildMeasuredValueJson.schema !== 'bitevo.build-measured-value-gate/v1' || buildMeasuredValueJson.state !== 'PUBLIC_BUILD_MEASURED_VALUE_READINESS_TEMPLATE_NOT_CUSTOMER_STATE' || buildMeasuredValueJson.customer_state !== 'NOT_REPRESENTED') failures.push('/build/measured-value-gate.json: schema/state/customer-state drift');
for (const key of ['payment_received','payment_verified','delivery_started','delivery_completed','measurement_window_closed','measured_value_observed','customer_case_evidence','renewal_expansion_evidence','roi_claim_published','customer_result_claim_published']) if (buildMeasuredValueJson.claims?.[key] !== false) failures.push(`/build/measured-value-gate.json: ${key} must remain false`);
if (buildMeasuredValueJson.required_measurement_evidence?.length !== 8 || buildMeasuredValueJson.required_measurement_evidence?.some(item => item.status !== 'REQUIRES_EXTERNAL_EVIDENCE')) failures.push('/build/measured-value-gate.json: measurement evidence gate drifted');
if (buildMeasuredValueJson.synthetic_reference?.route !== '/build/workflow-value-example' || buildMeasuredValueJson.synthetic_reference?.customer_evidence !== false || buildMeasuredValueJson.synthetic_reference?.measured_value_evidence !== false) failures.push('/build/measured-value-gate.json: synthetic reference boundary drifted');
...
for (const phrase of ['Измеренная ценность ещё не означает продление.','Никакой renewal или expansion state здесь не представлен.','Measured value не заменяет buyer renewal decision.']) if (!ruBuildRenewalExpansionText.includes(phrase)) failures.push(`/ru/build/renewal-expansion-gate: missing boundary phrase "${phrase}"`);
...
for (const phrase of ['Краткое описание границ ещё не является предложением.', 'TEMPLATE ONLY · NO PROPOSAL ISSUED · NO CONTRACT · NO INVOICE · NO PAYMENT RAIL · NO CHECKOUT · NO TESTING AUTHORIZATION', 'actual_proposal_issued=false']) {
...
for (const phrase of ['Предложение ещё не означает оплаченный старт.', 'Эта страница не может доказать payment.', 'Delivery по умолчанию остаётся unstarted.']) if (!ruAuditPaidStartText.includes(phrase)) failures.push(`/ru/audit/paid-start-gate: missing boundary phrase "${phrase}"`);
...
for (const phrase of ['Выполнение ещё не означает измеренную ценность.', 'Никакой delivery или measurement state здесь не представлен.', 'не заменяют measured value']) if (!ruAuditMeasuredValueText.includes(phrase)) failures.push(`/ru/audit/measured-value-gate: missing boundary phrase "${phrase}"`);
...
for (const phrase of ['Измеренная ценность ещё не означает продление.', 'Никакой renewal или expansion state здесь не представлен.', 'Measured value не заменяет buyer renewal decision.']) if (!ruAuditRenewalExpansionText.includes(phrase)) failures.push(`/ru/audit/renewal-expansion-gate: missing boundary phrase "${phrase}"`);
